...
var setErrorEmptyValues = function setErrorEmptyValues(tree, config) {
  var newTree = tree;

  function _processNode(item, path) {
    var itemPath = path.push(item.get('id'));
    var properties = item.get('properties');
    var ruleStr = properties.get('field');
    var rule;
    ...
    if (properties) {
      ...
      if (field) {
        if (value && value.toJS().length > 0 && value.toJS()[0] === undefined || value.toJS()[0] === '' || operator === 'between' && value.toJS()[1] === undefined) {
  ...
  function _checkNestedRule(children, newTree, itemPath, rule) {
    var requiredRules = _toConsumableArray(config.fields[rule[0]].subfields[rule[1]].requiredFields);
