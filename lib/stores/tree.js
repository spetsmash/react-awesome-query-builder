...
var removeRule = function removeRule(state, path, config) {
  ...
  if (isEmptyGroup) {
    if (isParentRuleGroup) {
      state = state.deleteIn((0, _treeUtils.expandTreePath)(parentPath)); // taking the parent group

      var groupPath = parent.get('path').slice(0, -1);
      var groupItem = (0, _treeUtils.getItemByPath)(state, groupPath);
    ...
    var _rulesNumber = countRules(_index.Utils.getTree(state).children1);

    if (_rulesNumber === 0) {
      state = addItem(state, state.get('path'), 'rule', (0, _uuid["default"])(), (0, _defaultUtils.defaultRuleProperties)(config), config);
    }

    if (!isParentRuleGroup && !canLeaveEmpty && state.getIn((0, _treeUtils.expandTreePath)(parentPath)).get('children1').size === 0) {
      state = addItem(state, parentPath, 'rule', (0, _uuid["default"])(), (0, _defaultUtils.defaultRuleProperties)(config, parentField), config);
      state = state.deleteIn((0, _treeUtils.expandTreePath)(parent.get('path')));
...
var moveItem = function moveItem(state, fromPath, toPath, placement, config) {
  ...
  var groupPath = from.get('path').slice(0, -1);
  var groupItem = (0, _treeUtils.getItemByPath)(state, groupPath);
...
var setField = function setField(state, path, newField, config) {
  ...
  return state.updateIn((0, _treeUtils.expandTreePath)(path, 'properties'), function (map) {
    return map.withMutations(function (current) {
      ...
      var currentOperatorOptions = current.get('operatorOptions'); // const _currentField = current.get('field');
...
var setOperator = function setOperator(state, path, newOperator, config) {
  return state.updateIn((0, _treeUtils.expandTreePath)(path, 'properties'), function (map) {
    return map.withMutations(function (current) {
      ...
      var _getNewValueForFieldO2 = (0, _validation.getNewValueForFieldOp)(config, config, current, currentField, newOperator, false, 'operator', true),
