...
var convertFromLogic = function convertFromLogic(logic, conv, config, expectedType, meta) {
  ...
  if (isRev) {
    ...
    ret = convertRuleGroup(op, vals, conv, config, not, meta) || convertConj(op, vals, conv, config, not, meta) || convertOp(op, vals, conv, config, not, meta);
  }
...
var convertRuleGroup = function convertRuleGroup(op, vals, conv, config, not, meta) {
  ///TODO
  var conjKey = conv.conjunctions[op];

  if (conjKey) {
    var resultObject = vals.reduce(function (acc, el) {
      return Object.assign(acc, el);
    }, {}); // console.log(resultObject)

    var rulesOnly = Object.keys(resultObject).some(function (el) {
      return el === 'and' || el === 'or';
    });
    var field = undefined;
